...
After you installed Ollama on your machine and downloaded the package rollama you can load the library by calling:
...
If you want to annotate textual data, you can use various prompting strategies. For an overview of common approaches, you can read a paper by [Weber and Reichardt (2023)](<https://arxiv.org/abs/2401.00284>). These strategies primarily differ in whether or how many examples are given (Zero-shot, One-shot, or Few-shot) and whether reasoning is involved (Chain-of-Thought).

When writing a prompt we can give the model content for the system part, user part and assistant part. The system message typically includes instructions or context that guides the interaction, setting the stage for how the user and the assistant should interact.
For an annotation task we could write: "You assign texts into categories. Answer with just the correct category." The table summarizes different prompting strategies for annotating textual data. Each strategy varies in the number of examples given and the incorporation of reasoning.
...
#> 
#> * The use of the word "tastes" implies that the speaker has a strong opinion about the taste of the pizza, which is typically associated with negative
#> emotions.
#> * The phrase "terrible" is a strong adjective that conveys a sense of disappointment or displeasure.
#> * The sentiment is further reinforced by the fact that the speaker used the word "tastes," which implies a personal evaluation or judgment about the pizza's
#> taste, rather than a neutral description.
#> 
#> Overall, based on the language and structure of the sentence, it seems clear that the speaker has a negative sentiment towards the pizza.
print(output_thought$message$content)
...
```
In the next step we can use the assistant's reasoning and an user prompt with the classification question.
...
create_query <- function(systemmsg, examples, texttoclassify, classification_question) {
  ...
  for(example in examples) {
    ...
    q <- tibble::add_row(q, role = "user", content = usermsg_with_question)
    q <- tibble::add_row(q, role = "assistant", content = example$assistantmsg)
  }

  # Add the current text to classify along with the classification question
  usermsg_final <- paste("text:", texttoclassify, "\n", classification_question)
  q <- tibble::add_row(q, role = "user", content = usermsg_final)
...
process_reviews <- function(reviews) {
  ...
  for (i in seq_along(reviews$review)) {
    ...
    output <- query(q)
    annotations[i] <- output$message$content
  }
