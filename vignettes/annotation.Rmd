...
After you installed Ollama on your machine and downloaded the package rollama you can load the package and pull the default model (`llama3.1`) by calling:
...
#> 
#> ── Answer from llama3.1 ─────────────────────────────────────────────
#> I would categorize this text as "negative". Here's why:
#> 
#> * The word "terrible" has a strong negative connotation and implies
#> that something is of poor quality.
#> * There are no positive words or phrases in the text to balance out
#> the negativity, such as "but..." or "however..." that might suggest
#> a more neutral or even positive sentiment.
#> * The sentence expresses a clear opinion or judgment about the
#> pizza's taste, which further reinforces the negative sentiment.
#> 
#> Overall, based on the language and tone used, it's clear that the
#> speaker is unhappy with their experience of eating this particular
#> pizza.
```
...
#> 5         5 The movie lacked depth, but the special effects were inc…
```

We can use `create_query` again to define a query for each of these reviews.
...
#>       <int> <chr>                                          <chr>     
#> 1         1 A stunning visual spectacle with a gripping s… {Positive}
#> 2         2 The plot was predictable, but the acting was … {Positive}
#> 3         3 An overrated film with underwhelming performa… {Negative}
#> 4         4 A beautiful tale of love and adventure, beaut… {Positive}
#> 5         5 The movie lacked depth, but the special effec… {Neutral}
```
