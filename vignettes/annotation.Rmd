...
#> 
#> ── Answer from llama3.2:3b-instruct-q8_0 ─────────────────────────────────
#> I would assign a sentiment of **negative** to this text.
#> 
#> My reasoning is as follows:
#> 
#> * The speaker explicitly states that the pizza "tastes terrible", which
#> implies a strong negative opinion.
#> * There are no positive adjectives or phrases used to describe the pizza,
#> such as "delicious" or "amazing". Instead, the focus is on the unpleasant
#> taste.
#> * This kind of direct statement often indicates a clear dissatisfaction
#> with the product (in this case, the pizza).
#> 
#> Overall, the text conveys a strong negative sentiment towards the pizza.
```
...
#>   role   content                                                          
#>   <chr>  <glue>                                                           
#> 1 system You assign texts into categories. Answer with just the correct c…
...
#> 5         5 The movie lacked depth, but the special effects were incredib…
```
...
All of them have the same prompt, system message and prefix, but each has a different text that came from the movide reviews data.frame we created above.
...
#>       <int> <chr>                                               <chr>     
#> 1         1 A stunning visual spectacle with a gripping storyl… positive  
#> 2         2 The plot was predictable, but the acting was super… Neutral   
#> 3         3 An overrated film with underwhelming performances.  negative  
#> 4         4 A beautiful tale of love and adventure, beautifull… Positive  
#> 5         5 The movie lacked depth, but the special effects we… Neutral
```
...
movie_reviews_annotated <- movie_reviews |>
  mutate(
    ...
    sentiment = query(queries, screen = FALSE, output = "text")
  )
```
