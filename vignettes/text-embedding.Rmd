...
#> $ `Clothing ID`             <dbl> 767, 1080, 1077, 1049, 847, 1…
#> $ Age                       <dbl> 33, 34, 60, 50, 47, 49, 39, 3…
#> $ Title                     <chr> NA, NA, "Some major design fl…
#> $ `Review Text`             <chr> "Absolutely wonderful - silky…
#> $ Rating                    <dbl> 4, 5, 3, 5, 5, 2, 5, 4, 5, 5,…
#> $ `Recommended IND`         <dbl> 1, 1, 0, 1, 1, 0, 1, 1, 1, 1,…
#> $ `Positive Feedback Count` <dbl> 0, 4, 0, 0, 6, 4, 1, 4, 0, 0,…
#> $ `Division Name`           <chr> "Initmates", "General", "Gene…
#> $ `Department Name`         <chr> "Intimate", "Dresses", "Dress…
#> $ `Class Name`              <chr> "Intimates", "Dresses", "Dres…
```
...
#> 1 -1.30 0.277  2.73   0.608   -1.95  -1.19   0.425 -0.349 0.935
#> 2  1.42 1.08  -0.155 -0.00261  0.304 -2.53  -0.592  1.95  2.08 
#> 3 -2.37 0.287 -0.891 -1.68     0.462 -0.742 -1.57   0.191 2.55 
#> # ℹ 3,063 more variables: dim_10 <dbl>, dim_11 <dbl>,
#> #   dim_12 <dbl>, dim_13 <dbl>, dim_14 <dbl>, dim_15 <dbl>,
#> #   dim_16 <dbl>, dim_17 <dbl>, dim_18 <dbl>, dim_19 <dbl>,
#> #   dim_20 <dbl>, dim_21 <dbl>, dim_22 <dbl>, dim_23 <dbl>,
#> #   dim_24 <dbl>, dim_25 <dbl>, dim_26 <dbl>, dim_27 <dbl>,
#> #   dim_28 <dbl>, dim_29 <dbl>, dim_30 <dbl>, dim_31 <dbl>,
#> #   dim_32 <dbl>, dim_33 <dbl>, dim_34 <dbl>, dim_35 <dbl>, …
```
...
#>  1     0 <5      1.12   1.56  -4.48 -0.129  -0.373  0.390 
#>  2     1 5       0.792  0.721 -3.14 -0.808  -1.81   1.35  
#>  3     2 <5      0.539  1.12  -2.58 -0.417  -0.992  1.77  
#>  4     3 5      -0.150  1.25  -4.12 -0.0750 -0.835  1.06  
#>  5     4 5       0.352  0.972 -3.40 -1.18   -0.686  0.489 
#>  6     5 <5      0.907  0.975 -2.78 -0.638  -1.48   2.21  
#>  7     6 5       0.523  0.321 -2.46 -0.678  -0.640  0.501 
#>  8     7 <5      0.224  0.694 -3.12 -0.562  -1.50  -0.0708
#>  9     8 5      -0.0477 1.21  -3.70 -0.300  -0.936  0.583 
#> 10     9 5      -0.105  1.13  -3.22 -0.310  -1.69   0.857 
#> # ℹ 4,990 more rows
#> # ℹ 762 more variables: dim_7 <dbl>, dim_8 <dbl>, dim_9 <dbl>,
#> #   dim_10 <dbl>, dim_11 <dbl>, dim_12 <dbl>, dim_13 <dbl>,
