...
- images:
  ...
  path(s) to images (for multimodal models such as llava). A plain
  character vector of paths/URLs is attached to every query as-is (e.g.
  to ask about several images at once). To annotate several images
  individually with the same prompt, pass a **list** instead, e.g.
  `images = as.list(paths)`: each list element is then paired up with
  `q` (recycling whichever of the two has length 1) to produce one query
  per image.
