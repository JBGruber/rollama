...
test_that("Test chat", {
  ...
  expect_equal(
    chat_history()$content[c(1, 3)],
    c("Please only say 'yes'", "One more time")
  )
  expect_equal(chat_history()$role, c("user", "assistant", "user", "assistant"))
  expect_error(
    query(q = tibble::tibble(role = "assistant", content = "Pos")),
...
test_that("Test inputs", {
  ...
  expect_error(
    query(data.frame(role = "system", content = "test")),
...
test_that("Test output parameter", {
  ...
  expect_error(
    query("Please only say 'yes'", output = "invalid"),
    "should.be.one.of"
  )
  ...
  expect_s3_class(
    query("Please only say 'yes'", screen = FALSE, output = "httr2_response")[[
      1
    ]],
    "httr2_response"
  )
  expect_equal(
    names(query("Please only say 'yes'", screen = FALSE, output = "list")[[1]]),
    c("request", "response")
  )
  expect_equal(
    colnames(query(
...
test_that("Test stream", {
  skip_if_not(ping_ollama(silent = TRUE))
  snapshot <- query("test", model_params = list(seed = 42), stream = FALSE)
  snapshot2 <- query("test", model_params = list(seed = 42), stream = TRUE)

  expect_equal(names(snapshot[[1]]), names(snapshot2[[1]]))
  expect_equal(rapply(snapshot, typeof), rapply(snapshot2, typeof))
  expect_equal(snapshot[[1]]$message, snapshot2[[1]]$message)
})
